...
const  store = $('.store'),
    ...
    cellUpdateDOM = function (key) {
        ...
        const $cell = store.children('.cell').eq(key);
    ...
    saveState = function (cellsArray) {
        localStorage.setItem('store-' + getStorageKeySuffix(), JSON.stringify(cellsArray));
    },
    loadState = function () {
        let result = [];

        try {
            result = JSON.parse(localStorage.getItem('store-' + getStorageKeySuffix()));
        } catch (e) {
