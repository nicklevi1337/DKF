...
const table = $('.table'),
    ...
    cellUpdateDOM = function (key) {
        ...
        const $cell = table.children('.cell').eq(key);
    ...
    saveState = function (cellsArray) {
        localStorage.setItem('table-' + getStorageKeySuffix(), JSON.stringify(cellsArray));
    },
    loadState = function () {
        let result = [];

        try {
            result = JSON.parse(localStorage.getItem('table-' + getStorageKeySuffix()));
        } catch (e) {
