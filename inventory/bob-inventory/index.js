...
const list = $(".bob__inventory"),
  ...
  selectUnit = function (key) {
    ...
    const unit = $(".unit", list).removeClass("active");
  ...
  saveState = function (unitArray) {
    localStorage.setItem(
      "bob__inventory-" + getStorageKeySuffix(),
      JSON.stringify(unitArray)
    );
  },
  loadState = function () {
    let result = [];
    ...
    try {
      result = JSON.parse(
        localStorage.getItem("bob__inventory-" + getStorageKeySuffix())
      );
    } catch (e) {
