...
const bagpack = $(".bagpack"),
  ...
  cellUpdateDOM = function (key) {
    ...
    const $cell = bagpack.children(".cell").eq(key);
  ...
  selectCell = function (key) {
    ...
    const cells = $(".cell", bagpack).removeClass("active");
  ...
  saveState = function (cellsArray) {
    localStorage.setItem(
      "bagpack-" + getStorageKeySuffix(),
      JSON.stringify(cellsArray)
    );
  },
  loadState = function () {
    let result = [];
    ...
    try {
      result = JSON.parse(
        localStorage.getItem("bagpack-" + getStorageKeySuffix())
      );
    } catch (e) {
